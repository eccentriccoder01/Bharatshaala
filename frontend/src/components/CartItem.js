...
const CartItem = ({ item, onQuantityChange, onRemove }) => {
  ...
  return (
    <div className={`flex items-center space-x-4 p-4 bg-white dark:bg-gray-800 rounded-xl border border-emerald-200 dark:border-emerald-700 transition-all duration-300 ${isRemoving ? 'opacity-50 scale-95' : 'hover:shadow-md'
      }`}>
      ...
      <div className='flex items-center space-x-3'>
        <label className='text-emerald-700 dark:text-emerald-300 font-medium text-sm'>{t('quantity')}:</label>
        <select
          value={item.Quantity}
